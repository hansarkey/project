...
categories = list(set([line.split(':')[0] for line in open('city_samples.txt').readlines()]))
print(categories)
...
stubsPerCat = int(len(open('city_samples.txt').readlines()) / len(categories))


counter = 0
i = 0
datfile = open('city_samples.txt', mode ='r')
for line in datfile:
    line = line.split(':')[1]
    line = line.replace('\n', ' ')
